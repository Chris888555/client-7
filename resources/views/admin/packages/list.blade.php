...
<div id="packageTable" class="mt-6">
    ...
    <div id="editFormContainer" style="display: none;" class="mt-6 bg-white p-6 rounded-xl border">
        <form id="updatePackageForm" method="POST" action="{{ route('packages.update') }}" class="space-y-4">
            @csrf
            ...
            features.forEach(f => {
                featureWrap.innerHTML += `
                    <input type="text" name="features[]" value="${f}" 
                        class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:ring-gray-200 mb-2" required>`;
            });
