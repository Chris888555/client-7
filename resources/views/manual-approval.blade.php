...
<div class="flex flex-col md:flex-row gap-4 mb-4">
    ...
    <tr class="border-b">
        ...
        <td class="px-4 py-2 text-sm">₱{{ number_format($funnel->plan_price, 2) ?? '0.00' }}</td>
        <td class="px-4 py-2 text-sm capitalize">
            ...
            @else
            <span class="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm">{{ ucfirst($funnel->status) }}</span>
            @endif
        </td>
        <td class="px-4 py-2 text-sm" x-data="{ openModal: false }">
            @if($funnel->proof_image)
            <img src="{{ asset('storage/' . $funnel->proof_image) }}" width="50" alt="Proof Image" class="rounded-md border cursor-pointer" @click="openModal = true" />
            @else
            No proof image
            @endif
            <!-- Modal -->
            <div x-show="openModal" x-transition class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center" @click="openModal = false">
                <div class="bg-white p-4 rounded-md w-[95%] sm:w-[700px]">
                    <div class="flex justify-end">
                        <button @click="openModal = false" class="text-gray-500 border-2 border-gray-100 rounded-lg">
                    ...
                    <img src="{{ asset('storage/' . $funnel->proof_image) }}" alt="Proof Image" class="w-full rounded-md mt-4">
                </div>
            </div>
        </td>
        @endif
